...
if(isset($_GET['filter']) && $_GET['filter']=='city') {
  ...
  } else if(isset($_GET['filter']) && $_GET['filter']=='state') {
      
      $sql = "SELECT * FROM job_post WHERE state>='$_GET[search]' LIMIT $start_from, $limit";
    ...
    while($row = $result->fetch_assoc()) {
      $sql1 = "SELECT * FROM company WHERE id_company='$row[id_company]'";
         ...
         <div class="attachment-block clearfix">
                <img class="attachment-img" src="uploads/logo/<?php echo $row1['logo']; ?>" alt="Attachment Image">
                <div class="attachment-pushed">
                  <h4 class="attachment-heading"><a href="view-job-post.php?id=<?php echo $row['id_jobpost']; ?>"><?php echo $row['jobtitle']; ?></a> <span class="attachment-heading pull-right">$<?php echo $row['maximumsalary']; ?>/Month</span><hr/><a href='<?php echo $row1['website']; ?>' style="float:right;"><?php echo $row1['website']; ?></a></h4>
                  <div class="attachment-text">
                      <div><strong><?php echo $row1['companyname']; ?> | <?php echo $row1['city']; ?> | Experience <?php echo $row['experience']; ?> Years</strong></div>
                  </div>
                </div>
